...
class SectionProcessing(Resource):
    def post(self):
        try:
            ...
            if not image_filename.startswith("Specimen_") or not image_filename.endswith(".tif"):
            ...
            print(image_path)
            sp_name = image_filename.split('.')[0]  # Using the filename directly since we've already validated it
            ...
            return {'message': 'Image processed successfully', 'crops': crops}, 200
...
class CountProcessing(Resource):
    def post(self):
        try:
            data = request.get_json()
            occlusion_model_path = 'models/occlusion.onnx'
            pillar_filter_path = 'models/pillar_filter.onnx'
            specimen_name = 'Specimen_001'

            sections_image_path = data.get('sections_image_path')
            ...
            for sec_id, section_path in enumerate(sections_image_path):
                ...
                image, pillar_count, all_points = process_pillar.detect_pillars(section_image, patch, sec_id, specimen_name)

                # Find occlusions in the section
                num_negative, num_positive = occlusion_model.occlusion_finder(all_points, section_image, sec_id, specimen_name)

                print(f'Section {4 - sec_id}: Number of pillars: {pillar_count}, Occlusion count: {num_positive}')

                return f'Section {4 - sec_id}: Number of pillars: {pillar_count}, Occlusion count: {num_positive}', 200
        except Exception as e:
            import traceback
            traceback.print_exc()
            return {'message': f'Error processing image: {str(e)}'}, 500
